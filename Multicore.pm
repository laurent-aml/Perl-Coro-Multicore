...
thread, so it needs a perl that is not I<using> ithreads. On Windows that means a
non-ithread build, because the threaded build there turns on C<PERL_IMPLICIT_SYS>
and routes even C<malloc> through a per-interpreter table. As Windows' C<fork>
emulation is built on ithreads as well, such a perl has no C<fork> at all, which is
what choosing that build costs. Offload does not care either way. The cost is that the XS module hands its C call
...
=item An offload never completes if nothing lets the event loop run

Offload completion is delivered through this module's wakeup pipe, which the event
loop watches. So an offloaded call returns only once the loop gets a turn. The work
itself runs on the worker either way, but C<done> cannot run, and C<get> cannot
return, until something polls the loop.

In practice this costs nothing, because a program using this module is running an
event loop anyway - and it does not matter which thread issues the offload. Issuing
one from the main thread is fine: while it waits, Coro finds nothing else runnable
and its idle handler runs the loop, which is exactly what delivers the completion.

What does break it is a Coro thread that stays I<runnable> without ever blocking:

   async { while (1) { $ticks++; cede } };   # starves the loop

C<cede> passes control to another I<ready> thread, never to the loop, so with such a
thread alive the loop is never polled and an outstanding offload hangs - the value
computed and waiting, with nobody to collect it.

This is not really an offload limitation: the same thread stalls the loop's timers
and I/O watchers just as thoroughly. It is called out here only because the symptom
is different - one call that never returns, rather than a program that feels slow -
and that is easy to misread as a deadlock in this module. There is deliberately no
timeout: a legitimate offload may take arbitrarily long, so there is no duration at
which waiting could be called an error. If you need to spin, spin on something that
blocks (C<Coro::AnyEvent::sleep 0>, C<Coro::AnyEvent::idle>) rather than C<cede>.
