 ...
 * Windows.
 *
 * This backend is the STACKFUL (Coro) delivery: multicore_offload() queues `work`
 * on a worker and returns a PENDING handle at once.  A consumer that asks the
 * handle for the value - which multicore_offload_sync () does for it - suspends the
 * calling Coro until the work is over, so the offloaded call still looks
 * synchronous to its caller while the other Coro threads run meanwhile.  A consumer
 * that returns the handle instead gives its own caller an asynchronous entry point,
 * and one Coro can then hold several offloads in flight.  (A stackless
 * Future::AsyncAwait backend implements the same hook by resolving a Future from
 * done () on its loop; the handle contract is the same either way.)
 *
 ...
 * in parallel), the worker signals the pipe on completion, and poll() runs done ()
 * and resolves the handle, which readies whoever is waiting on it.
 *
